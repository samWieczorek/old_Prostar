...
\RequirePackage{C:/Users/FC219170/Documents/R/R-3.4.1/library/BiocStyle/resources/tex/Bioconductor}

\AtBeginDocument{\bibliographystyle{C:/Users/FC219170/Documents/R/R-3.4.1/library/BiocStyle/resources/tex/unsrturl}}
...
The Gene Ontology (www.geneontology.org) is a controlled vocabulary for
annotating 3 aspects of a gene product's biology. This ontology is made of 3 
parts : Molecular Function (MF), Biological Process (BP) and Cellular Component 
(CC). 

The Gene Ontology (GO) analysis is the last step proposed in the "Data 
processing" menu. It aims to provide the user to have a "global" view of what is 
...
We used the package \Biocpkg{clusterProfiler} to perform 2 types of GO 
analysis : Classification and Enrichment. The GO analysis section in 
\Biocpkg{ProStar} is separated in 4 tabs (see Fig.~\ref{fig:GO_tab1}): 
\begin{itemize}
\item GO Setup 
...
The left-hand side of the \textbf{GO Setup} tab allows to set the input parameters, which
are : 
\begin{itemize}
\item Source of protein ID: user indicates either a column in the current 
dataset or chooses a file (1 ID per line format). 
...
\item Ontology: which level of the ontology to work with. 
\end{itemize}


Once these parameters filled, clicking "Map proteins IDs" launches the  mapping
of the user IDs on the annotation package, and thus on the Gene Ontology 
categories. 
On the right-hand side of the panel is then indicated the proportion of proteins 
that have been impossible to map with the annotation package (informative 
ouput). 
...
Next step is to perform either Classification or Enrichment GO analysis (or 
both). 
...
The graphics shows the most represented GO categories for the user-defined 
ontology at the user-defined level(s).
...
The \textbf{GO Enrichment} tab (see Fig.~\ref{fig:GO_tab3}) allows to know which
GO categories are significantly enriched in the list the user provided, compared 
to a reference ('background' or 'universe') the user defined. 
This background can be : 
...
(ii) the entire dataset loaded in ProStaR, $i.e.$ in the example of this 
tutorial: all the proteins IDs of the 'Leading\_razor\_protein' column of the 
MSnSet, or 

(iii) a custom IDs list provided in a file by the user (one ID per line format). 
...
\Biocpkg{clusterProfiler} package, which allows to define a threshold to 
consider a GO category as significant or not. 
This threshold is a user-defined value for the (FDR) adjusted $p$-value, set in 
the 'FDR (BH Adjusted $P$-value cutoff)' field. 
Analysis is launched by clicking the "Perform enrichment analysis" button.

The result consists on 2 graphics shown in the right-hand side of the panel 
once the analysis has been done (see Fig.~\ref{fig:GO_tab3}). 
First one (top) is a barplot showing the 5 most significant categories. The 
length of the bar represents the number of proteins under the category. 
Second one (bottom) is a dotplot ranked by decreasing GeneRatio. GeneRatio is, 
for a given category, the ratio between 
(Genes of the input list in this category)/(Total number of Genes in the
category). 
...
The last tab is the \textbf{Save GO analysis} one. It allows to save results of
what has been performed :  GO classification, GO enrichment, or both 
(see Fig.~\ref{fig:GO_tab4}).
...
\begin{itemize}\raggedright
  \item R version 3.4.1 (2017-06-30), \verb|x86_64-w64-mingw32|
  ...
  \item Loaded via a namespace (and not attached): backports~1.1.1,
    BiocStyle~2.4.1, compiler~3.4.1, digest~0.6.12, evaluate~0.10.1,
    htmltools~0.3.6, knitr~1.17, magrittr~1.5, Rcpp~0.12.12, rmarkdown~1.6,
    rprojroot~1.2, stringi~1.1.5, stringr~1.2.0, tools~3.4.1, yaml~2.1.14
\end{itemize}
