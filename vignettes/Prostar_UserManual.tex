...
\Biocpkg{Prostar} relies on the \Biocpkg{clusterProfiler} pacakge to perform both GO 
...
%$$ \text{\textit{GeneRatio}} = \frac{\#(\text{Genes of the input list in this category})}{\#(\text{Total number of Genes in %the
%category})}.
%$$


\hl{\bf Command line:} The \Biocpkg{DAPAR()} function to perform the GO 
Enrichment is \Rfunction{enrich\_GO()}. It returns an 'enrichResult' instance.
