...
In addition to the exported datasets (see previous section), the user can generate and download a report dynamically created on demand (e.g. at the end of an analysis) (Fig.~\ref{fig:exportReport}).

The left part of the interface shows a list of checkboxes (one for each dataset created during the analysis). By default, all the datasets are selected. The user has to choose which results have to be included in the report. Even if no analysis has been done (e.g. the upload of a dataset), the plots of the Descriptive Statistics are available.

The right part of the interface is to let the user choose the format of the report; it can be a pdf, a HTML or a Word file. He/she has also to specify the name of the file.

Once those options have been choosen, the user clicks on the button called "Generate a report". Then, ProStaR creates all the images needed and include them in the report file. During this step, the button "Download" is disabled. It become enabled only when the report if ready (in background) to be downloaded.
...
As this feature is in its Bêta version, some functionalities are not implemented yet:
\begin{itemize}
\item The R commands run during the generation of the report are not shown in the log console (See \ref{sec:sessionlog}).
\end{itemize}
...
proteins for each condition (see Fig.~\ref{fig:sdvd}). For better visualization, 
the user can zoom in by click-and-drag.
...
\Biocpkg{norm} or a deterministic quantile (xxxxxxxx)
\end{itemize}
...
X-axis. Regarding the computation of the $p$-values, one test is available 
in \Biocpkg{DAPAR}: the moderated $t$-test (from package \Biocpkg{limma}).
...
\hl{\bf Command line:} The \Biocpkg{DAPAR} function for the moderated $t$-test 
is \Rfunction{diffAnaLimma()}. This function return a 
