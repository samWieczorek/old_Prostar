...
\item {Perform the various processing steps of a complete data analysis}: (i) filtering and data cleaning; (ii) cross-replicate normalization; (iii) missing value imputation; {(iv) aggregation of peptide intensities into protein intensities;} (v) statistical tests and false discovery rate computation.
\end{itemize}
...
% c'est trop tot pour indiquer cela: La version correspondante de Proline n'est pas encore une release
...
\begin{itemize}
\item {From CRAN}: \CRANpkg{RColorBrewer}, \CRANpkg{Cairo}, \CRANpkg{png}, \CRANpkg{lattice}, \CRANpkg{reshape2}, \CRANpkg{tmvtnorm}, \CRANpkg{norm}, \CRANpkg{ggplot2}, \CRANpkg{imputeLCMD}, \CRANpkg{gplots}, \CRANpkg{XLConnect}, \CRANpkg{knitr}, \CRANpkg{cp4p},\CRANpkg{doParallel}, \CRANpkg{scales}\CRANpkg{stats}, \CRANpkg{grDevices}
...
This step will automatically install the following packages: \CRANpkg{shiny}, \CRANpkg{reshape2}, \Githubpkg{jrowen/rhandsontable}, \CRANpkg{quantmod}, \CRANpkg{data.table}, \CRANpkg{DT}, \CRANpkg{shinyjs}.
...
\textcolor{red}{As illustrated on Fig.~\ref{fig:vuegal}, the interface has a classic menu Shiny layout with two panels:
\begin {itemize}
  \item \textbf {Upper panel}: A navbar menu to navigate throught the DAPAR functionalities and to run them;
  \item \textbf {Bottom panel}: The place where the inputs and outputs of the various processing are dispayed.
\end{itemize}
}
\begin{figure}
\centering
...
\textcolor{red}{More precisely, the navigating bar on the upper part of the screen is divided into four submenus:
\begin{itemize}
...
\textcolor{red}{At the right of these menus, a dropdown menu referred to as "Version of datasets" makes it possible to navigate back through the history of the processing. Its use is detailed in Section~\ref{sec:availabledatasets}
}
%\begin{figure}
%\centering
...
The "Data processing" menu contains the 5 predefined steps of a data analysis. They are designed to be used in a specific order:
\begin{enumerate}
\item {Filtering}
\item{Normalization}
...
Finally, once the ultimate tuning is chosen, the processing is applied to the dataset. In order to finalize the step, it is possible to save the result, so that another dataset appears in the "Version of dataset" list (see Section~\ref{sec:availabledatasets}).
...
\end {figure}
\textcolor{red}{Once done, the menu of "Version of dataset" is updated to "Original - peptide" or "Original - protein" whether the file contains quantitative information about peptides or proteins (see Section~\ref{sec:availabledatasets}).} All the plots in the "Descriptive statistics" submenu (see Section~\ref{sec:descriptivestatistics}) become accessible and all the widgets to interact with \Biocpkg{ProStaR} are preloaded.
...
\textcolor{red}{The user can find an example of MSnSet file in the installation directory of \Biocpkg{DAPAR}:\newline
...
\textcolor{red}{The user can download the plots showed in \Biocpkg{Prostar} by right-clicking on the plot. A contextual menu appears and let the user choose either "Save image as" or "Copy image". In the latter case, he/she has to paste the image in an appropriate software.}
...
\textcolor{red}{Two options are available to custom the plot:
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Version of dataset}\label{sec:availabledatasets}
\textcolor{red}{A major element of the menu entitled "Version of dataset" is detached from the menus, for it is convenient to have a continuous view on it. This is a  drop-down menu that lists between the different datasets, depending on the progress of the data analysis. Basically, each time the modifications of the current dataset are saved, the new dataset does not overwrite the previous one. On the contrary, the different versions are stored in memory. Thus, \Biocpkg{ProStaR} keeps a history of all processing performed on a dataset. Concretely, right after creating or uploading a dataset, only a single dataset is available: it is named "Original (peptide)" or "Original (protein)" whether the data are related to peptides or proteins. This information is registered in the MSnSet file (the slot "typeOfData" of the function \Rfunction{experimentData(object)}).
After the filtering step, if the user saves his/her results, another dataset becomes available, named "Filtered (peptide)" or "Filtered (protein)". Similarly, after the saving of the normalization, of the imputation of missing values, of the agregation into proteins and the differential analysis, a new dataset is created and stored. Each time a new dataset is created, it is by default the one on which the processing goes on. However, the previous one is accessible through the "Version of dataset" drop-down menu.}

\textcolor{red}{At any time, the name of the current dataset and the type of data are displayed. If the user needs to return to a previous dataset (for example, the current dataset is "Imputed" and the user wants to return to "Filtered"), he/she chooses it in the select field. The dataset is then automatically loaded in memory and becomes the current one; the new dataset becomes the new current one. Naturally, all the plots that are displayed throughout the various panels of \Biocpkg{ProStaR} are dynamically updated without any action from the user.}
...
\item For a refined analysis regarding the influence of a processing step, it also possible to switch from an older to a newer dataset (that has been saved before) with the "Version of dataset" drop-down menu, and to observe the variations in the "Descriptive statistics" menu.
\end{itemize}
...
In this section, the five steps of a quantitative data analysis are detailed. At the end of each step, it is advised to save the processing, so that another dataset shows up in the "Version of dataset" menu, and so that it is possible to go back to a previous step of the analysis if necessary, without starting back the analysis from scratch.
...
{In this step, the user may decide to delete peptides or proteins where the amount of missing values is too important to expect confident processing and/or if they are identified as contaminants or reverse peptides/proteins (that is artificial reversed amino acid sequences that are generated in a target-decoy approach).}
...
The user can vizualize the effect of filter options without changing the current dataset by clicking on "Perform filtering". If the filtering does not produce the expected effect, the user can test another one. To do so, one simply has to choose another method in the list and click again on "Perform filtering". The plots are automatically updated. This action does not modify the dataset but offers a preview of the filtered data. The user can vizualize as many times he/she wants several filtering options. Once he finds the correct one, he/she can choose to remove contaminants and reverses in the second tab called "String based filtering").

The user can choose to remove contaminants and/or reverse peptides or proteins. To do so, he/she selects the appropriate columns of the metadata listed in the dropdown menus. Then, he/she specifies in each of these columns the prefix that identifies the analytes to filter.

\textbf{Remark:} If he/she has no idea of the prefixes, he/she can switch to the Data Explorer in the Descriptive Statistics menu. 
...
\textcolor{red}{Once the the filtering is appropriately tuned, the user goes to the last tab (called "Vizualize and Validate") (see Fig.~\ref{fig:filter3}). Here, he can see the set of lines that have been previously filtered. In the left panel, he chooses among the lines filtered on missing values, contaminants or reverse ; then the data are diplayed in a table on the right panel. Finally, he clicks on "Apply filter" so as to validate his/her choice and to apply it to the dataset. Then, a new dataset is created. The informations related to the type of filtering and the options choosen appear in the Session log tab (see section~\ref{sec:sessionlog}). This latter becomes the new current dataset and its name appears in the menu \textbf{Version of dataset} at the top of the screen. All plots and tables are automatically updated in \Biocpkg{ProStaR}.}

\textcolor{red}{At this time, there is no version that works directly on dataframes.}
...
The user can vizualize as many times he/she wants several normalization methods. Once he finds the correct one, he/she validates his/her choice by clicking on "Save normalization". Then, a new "normalized" dataset is created and loaded in memory. The method of normalization that has been used is added to the Session log tab (see section~\ref{sec:sessionlog}). It becomes the new current dataset and the name "Normalized" appears in "Version of dataset". All plots and tables in other menus are automatically updated.

%\hl{\bf Command line:} In \Biocpkg{DAPAR}, the corresponding function is \Rfunction{normalizeD()}.
\textcolor{red}{\hl{\bf Command line:} In \Biocpkg{DAPAR}, the corresponding functions for the plot "before-after" normalization are:
\begin{itemize}
\item for the dataframe parameter: \Rfunction{compareNormalizationD()} ,
\item for an object of class MSnSet: \Rfunction{wrapper.compareNormalizationD()} .
\end{itemize}}
...
The user can vizualize as many times he/she wants several imputation methods. Once he finds the correct one, he/she validates his/her choice by clicking on "Save imputation". Then, a new "imputed" dataset is created and loaded in memory. The method of imputation used is added to the Session log tab (see section~\ref{sec:sessionlog}). This new dataset becomes the new current dataset and the name "Imputed" appears in "Version of dataset". All plots and tables in other menus are automatically updated.

\textcolor{red}{\hl{\bf Command line:} In \Biocpkg{DAPAR}, the function used to impute the missing values is \Rfunction{mvImputation()}. The two aforementioned plots are obtained with respectively:
\begin{itemize}
...
\textcolor{red}{
When working on a peptide datasets, one wants to switch to the protein level. This can be done with the agregation tool which aggregate the peptides into proteins. Thus, the peptide dataset become a protein dataset.
First, the user chooses the protein id of the dataset, i.e. the column that contains the IDs of proteins for each peptide. 
The two barplots represent the number of proteins that are identified by x peptides, respectively for the unique peptides only and with shared peptides. These are a view for the adjacency matrix used to aggregate the peptides.}
...
\textcolor{red}{The checkbox allows to select if the shared peptides must be taken into account or not in the aggregation process. The user can vizualize the effect of an aggregation method without changing the current dataset. If the aggregation does not produce the expected effect, the user can test another one. To do so, one simply has to choose another method in the list and click on "Perform agregation". The plot is automatically updated. This action does not modify the dataset but offers a preview of the agregated data. The user can vizualize as many times he/she wants several agregation methods. Once he finds the correct one, he/she can switch to the next tab called "Configure protein dataset".} 

\textcolor{red}{In this tab, he/she selects the columns of the peptide dataset that are of interest to be kept in the protein dataset (e.g. the sequence of the peptides). The effect of this action is to compile, for a given protein, the information of all of his peptides ; those informations are written in a colum of the protein dataset. Once done, he/she validates his/her choice by clicking on "Save agregation". Then, a new "agregated" dataset is created and loaded in memory. The method of agregation used is added to the Session log tab (see section~\ref{sec:sessionlog}). This new dataset becomes the new current dataset and the name "Agregated" appears in "Version of dataset". All plots and tables in other menus are automatically updated.
...
\textcolor{red}{The differential analysis is divided into four steps, each step correspond to a different tab:
\begin{itemize}
\item Selection of the statistical test and setting of a threshold on theFold Change,
...
\textcolor{red}{\textbf {Calibration step}. TO DO}
...
\textbf {Selection of statistical test, p-Value}: A threshold along the p-Value axis can be tuned by the user, so as to discriminate the differentially abondant proteins (which are colored in orange). A horizontal straight line is drawn to vizualize the threshold.
...
It becomes the new current dataset and its name, "DiffAnalysis.<test>" (where <test> indicates the test performed), appears in "Version of dataset". All plots and tables in other menus are automatically updated.
...
\textcolor{red}{Both packages \Biocpkg{DAPAR} and \Biocpkg{Prostar} are under active development. Despite our efforts to fix bugs, it may still have some cases where a fatal errors. For bugs, typos, suggestions or other questions, you are welcome to contact us by providing as much information as possible, a reproducible example and the output of sessionInfo(). Here are some error messages the user may encounter and the tip to work around.}

\textcolor{red}{\textbf{Unable to load loadWorkbook}. This error may appear if the package \CRANpkg{XLConnect} is not properly installed. First check if this package is well installed. The package \CRANpkg{XLConnect} needs the package \CRANpkg{rJava} to be installed. It may happen that the installation of rJava fails because of the configuration of environment variables (especially on Windows systems). In this case, try to set the variable JAVA\_HOME to an empy string before installing \CRANpkg{rJava} and \Biocpkg{DAPAR}:}
