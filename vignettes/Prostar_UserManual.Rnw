...
spectrometry). \textcolor{red}{The experiment package \Biocpkg{DAPARdata} is 
...
\textcolor{red}{The installation of the package \Biocpkg{DAPARdata} follows 
...
0.5 (median) or Other (In that case, the user can choose its personal 
quantile value).\newline
\end{itemize}
...
to rescale the dataset by mean of the variance reduction.
\end{itemize}
