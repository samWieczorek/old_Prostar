...
Once those options have been choosen, the user clicks on the button called "Generate a report". Then, ProStaR creates all the images needed and include them in the report file. During this step, the button "Download" is disabled. It becomes enabled when the report is ready to be downloaded.
