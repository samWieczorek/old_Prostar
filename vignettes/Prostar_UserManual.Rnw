...
is indicated (this informative 
ouput does not interrupt the process, unless no protein maps). 


\hl{\bf Command line:}


Next step is to perform either GO Classification or GO Enrichment (or 
both). 
...
GO categories are significantly enriched in the user's list, compared 
...
(ii) the entire dataset loaded in ProStaR (e.g. all the proteins IDs of the 'Leading\_razor\_protein' column of the 
dataset, as illustrated on Fig.~\ref{fig:GO_tab1}), or 
