...
\textcolor{red}{Note: the package XLConnect needs the package rJava to be installed. It may happen that the installation of rJava fails because of the configuration of environment variables.  If this arrives, one solution is to set JAVA\_HOME to before installing DAPAR:}
