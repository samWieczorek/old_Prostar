...
To expand the "Data processing" menu, click on the small triangle on the left. It contains the 5 predefined steps of a data analysis (see Fig.~\ref{fig:expandedMenu}). They are designed to be used in a specific order:
\begin{enumerate}
\item {Filtering}
\item{Normalization}
...
%~\cite{}.
\textcolor{red}{The user can load previously existing MSnSet files (see Section~\ref{sec:load}) or import text (-tabulated) and Excel files (see Section~\ref{sec:import}).}
...
\end {figure}
\textcolor{red}{Once done, the menu of "Available datasets" is updated to "Original (peptide / protein)" whether the file contains quantitative information about peptides or proteins (see Section~\ref{sec:availabledatasets}.} All the plots in the "Descriptive statistics" submenu (see Section~\ref{sec:descriptivestatistics}) become accessible and all the widgets to interact with \Biocpkg{ProStaR} are preloaded.
...
\textcolor{red}{In this step, the user may decide to delete peptides or proteins where the amount of missing values is too important to expect confident processing and/or if they are identified as Contaminants or Reverse.}
...
\textcolor{red}{The user can choose to remove Contaminants and/or Reverse peptides or proteins. To do so, he/she selects the appropriate columns of the metadata listed in the dropdown menus. Then, he/she specifies the prefix present in each of these columns that mark the lines to remove.}
...
\textcolor{red}{For each fullfilled option, the barplot below is updated: it shows the proportion of quantitative data, Contaminants and Reverse. Once the choices are made, the user click on "Perform filtering contaminants" to remove corresponding lines.}
...
\textcolor{red}{\hl{\bf Command line:} In \Biocpkg{DAPAR}, the corresponding functions are:
\begin{itemize}
