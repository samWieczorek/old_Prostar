...
source(file.path(".", "modules/moduleNotaNumeric.R"), local = TRUE)$value
...
moduleHypothesisTestUI <- function(id){
  ns <- NS(id)
  tagList(
    moduleNavigationUI(ns("moduleProcess_HypothesisTest"))
  )
}
...
moduleHypothesisTest <- function(input, output, session, dataIn, screen.id, settings=NULL){
  ...
  observe({
    callModule(moduleNavigation, "moduleProcess_HypothesisTest", 
